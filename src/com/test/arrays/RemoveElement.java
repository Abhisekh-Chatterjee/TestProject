...
public class RemoveElement {
	...
	public static void main(String[] args) {
		...
		while(runner < nums.length) {
			...
			if(nums[runner] != val) {
				swap(nums, firstValIndex, runner);
				firstValIndex++;
			}
			runner++;
		}
		
		System.out.println("Size of array without Val : "+firstValIndex);
		Arrays.stream(nums).forEach(System.out::print);

	}
	
	private static void swap(int[] nums, int firstValIndex, int runner) {
		int tmp = nums[runner];
		nums[runner] = nums[firstValIndex];
		nums[firstValIndex] = tmp;
	}
